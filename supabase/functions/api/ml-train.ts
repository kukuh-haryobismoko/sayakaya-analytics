// Retrains the BigQuery ML models that ml.ts serves predictions from.
...
const TRAIN_MAX_BYTES = String(Deno.env.get('ML_TRAIN_MAX_BYTES_BILLED') || 50_000_000_000);
...
export interface RetrainStepResult { step: string; ok: boolean; ms: number; error?: string }
export interface RetrainResult { ok: boolean; steps: RetrainStepResult[]; finishedAt: string }
...
export async function retrainModels(): Promise<RetrainResult> {
  const steps: RetrainStepResult[] = [];
  for (const step of STEPS) {
    ...
    try {
      ...
      steps.push({ step: step.name, ok: false, ms: Date.now() - startedAt, error: (e as Error).message });
    }
  }
