...
const normalizeFundIds = (fundIds?: string[]): string[] => (Array.isArray(fundIds) ? fundIds : []).map((v) => String(v).trim()).filter(Boolean);
function fundIdsClause(params: Record<string, unknown>, col: string, ids: string[], joiner = ' AND '): string {
...
export const overviewFunds = (fundIds: string[] = []): Query => {
  const ids = normalizeFundIds(fundIds);
...
// Restricted to settled/successful statuses — same set as the referral
// eligibility report's "did they buy" check — so an expired/cancelled/
// pending_payment attempt never shows up on an investor's official statement.
...
// static claim.
export const userRecentTransactions = (userId: string, limit = 8): Query => ({
...
// "All registered users" (see schedules.ts) --------------------------------
...
export const allInvestorsWithAum = (): Query => ({
  sql: `WITH latest AS (
...
export const allRegisteredUsersWithEmail = (): Query => ({
  sql: `SELECT u.id AS user_id, u.sid_code AS sid, u.email
    FROM ${USERS} u
    WHERE u.email IS NOT NULL`,
  params: {},
...
function aumRange(minAum?: number | string, maxAum?: number | string) {
  return {
    min: Number(minAum) || 0,
...
export const hnwiTotal = (date: string, minAum?: number | string, maxAum?: number | string, limit: number | string = 500): Query => {
  ...
  return {
    sql: `${cte}
      ...
      LIMIT @limit`,
    params: { ...params, minAum: min, maxAum: max, limit: parseInt(String(limit), 10) || 500 },
  };
};
...
export const hnwiByFund = (
  date: string,
  minAum?: number | string, maxAum?: number | string,
  minFundAum?: number | string, maxFundAum?: number | string,
  limit: number | string = 5000,
): Query => {
  const { cte, params } = hnwiBase(date);
  ...
  return {
    ...
    params: {
      ...
      limit: parseInt(String(limit), 10) || 5000,
    },
  };
};
...
// instead of per-fund/manager rows. Joined to FUNDS and filtered to
// listing_status='ACTIVE' — a liquidated fund stops being a real ongoing
...
function activeCte(params: Record<string, unknown>, ids: string[]): string {
  return `active AS (
...
// see the two routes in index.ts. Same -1-day grace window either way. This
// must stay in sync with the corresponding referralInviterStats*()
...
export const referralProgramDetail = (periodFrom?: string, periodTo?: string, inviteeDateField: 'created_at' | 'verified_at' = 'created_at'): Query => {
