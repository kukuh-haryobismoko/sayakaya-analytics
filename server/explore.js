...
// Registry of browsable datasets for the Data Explorer. All SQL is server-defined;
// only values (search text, filter choices, dates, paging) come from the user via
// named parameters. users.password and the sensitive KYC fields (id_number,
// mothers_maiden_name, photo/selfie/signature URLs, full home address) are never exposed.
...
function buildWhere(d, q, params) {
  const clauses = [];
  ...
  for (const f of d.filters) {
    ...
    params[f.key] = f.bool ? (v === 'true' || v === true) : v;
  }
...
function filterValuesSql(datasetKey, filterKey) {
  const d = DATASETS[datasetKey];
  if (!d) throw new Error('Unknown dataset.');
