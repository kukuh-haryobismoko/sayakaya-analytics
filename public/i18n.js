...
const I18N = {
  en: {
    ...
    refprog_alt_leaderboard_hint: `One row per inviter. Unlike the main Referral program tab, "Invited" here doesn't require the invitee to have signed up during the period — the referral itself can be arbitrarily old. What matters is their first-ever transaction: not made yet still counts as invited (pending), but one that already happened outside the period permanently excludes them.`,
    refprog_invited_title: `Invited users`,
    refprog_invited_hint: `One row per invitee behind the leaderboard's "Invited" count — their own account details, referral code, the referrer code they signed up with, KYC status, and first-transaction status.`,
  ...
  id: {
    ...
    refprog_alt_leaderboard_hint: `Satu baris per pengundang. Berbeda dari tab Referral program utama, "Invited" di sini tidak mensyaratkan teman yang diundang mendaftar dalam periode ini — referralnya sendiri boleh sudah sangat lama. Yang menentukan adalah transaksi pertamanya: belum pernah transaksi tetap dihitung invited (pending), tapi jika transaksi pertamanya sudah terjadi di luar periode ini, dia dikecualikan permanen.`,
    refprog_invited_title: `Pengguna yang diundang`,
    refprog_invited_hint: `Satu baris per teman yang diundang di balik angka "Invited" pada leaderboard — detail akunnya sendiri, kode referral miliknya, kode referrer yang dipakai saat mendaftar, status KYC, dan status transaksi pertamanya.`,
