...
const I18N = {
  en: {
    ...
    reset_password_ph: `New password`,
    ...
    docs_referral_program_desc: `The Sep-Dec 2026 referral bonus program's eligibility report. A referral only counts if the invited friend's very first-ever transaction is a Sucor Asset Management fund purchase of at least Rp1,000,000 made with someone's referral code; the bonus (Rp25,000 to each side, in Sucorinvest Money Market Fund units) additionally requires that fund's units to stay unchanged for 30 days. Each row shows both people's contact details, the transaction, days held, and a status — Eligible, Pending (still within the 30-day hold), or Not eligible (with the specific reason, e.g. redeemed early or KYC not verified). An Inviter leaderboard sits above the row-level detail: one line per inviter with how many people they referred and how many transacted at all (any fund, any amount) — broader counts than the eligibility rule itself — next to how many of those referrals qualified, and of those, how many are Pending vs. Eligible. On this leaderboard, "Invited" only counts an invitee if they also registered during the period — an old referral relationship that never leads to a signup/purchase in this window doesn't clutter the list. Below the leaderboard, an Invited users table lists that same "Invited" population one row per person — who invited them (SID and name), their own name, registration date, KYC verification date, SID, email, phone, their own referral code, the referrer code they signed up with, KYC status, and first-transaction status — separate from the row-level Referral detail table further down, which only covers invitees whose first-ever transaction met the campaign's own Sucor/≥Rp1jt rule.`,
    docs_referral_program_alt_desc: `Same eligibility rule, same detail rows, and same permission-independent tab as Referral program above — the only difference is the Inviter leaderboard's "Invited" definition (and, correspondingly, which invitees show up in the Invited users table below it). Here, "Invited" is gated by the invitee's KYC verification date instead of their registration date (with the same 1-day grace window) — an invitee who was verified inside the period counts as invited even if they registered long before it, and one who's never been verified never counts, no matter how old the referral. Use this tab when you want "who's KYC-verified and in play for this campaign," rather than "who signed up because of this campaign."`,
    ...
    docs_presentation_desc: `Internal meeting decks, viewable full-screen one page at a time, split into two sidebar tabs granted independently: AI Taskforce (July, September) and Monthly Review (August, September). Not shown to everyone — only accounts an admin has explicitly granted a tab to (e.g. a dedicated login for sharing at a meeting) can see it, same as any other restricted section.`,
    ...
    ss_send_portfolio: `Portfolio as of`,
    ss_send_statement: `Transaction e-statement for`,
    ...
    ss_log_title: `Send history`,
    ss_log_hint: `Every e-statement sent from this tab — who sent it, and to which investor. Most recent first.`,
    ss_batch_title: `Batch send`,
    ss_batch_hint: `Send the same portfolio/e-statement documents to multiple investors at once — enter one email or SID per line, or comma-separated.`,
    ss_batch_label: `Emails or SIDs`,
    ss_batch_modal_title: `Email batch statement`,
    sched_title: `Automated sending`,
    sched_new: `+ New schedule`,
    sched_hint: `Recurring sends — a schedule is only created after the person setting it up confirms a one-time code emailed to them.`,
    sched_modal_title: `New automated schedule`,
    sched_recipient_type: `Recipients`,
    sched_recipient_single: `Single email`,
    sched_recipient_csv: `Upload CSV / paste emails`,
    sched_recipient_all_aum: `All investors with AUM`,
    sched_recipient_all_registered: `All registered users`,
    sched_single_email_label: `Email address`,
    sched_csv_label: `Upload a CSV, or paste emails (one per line, or comma-separated)`,
    sched_send_portfolio: `Portfolio (live, as of send date)`,
    sched_send_statement: `Transaction e-statement (previous month)`,
    sched_frequency: `Frequency`,
    sched_freq_daily: `Daily`,
    sched_freq_weekly: `Weekly`,
    sched_freq_monthly_last: `End of month`,
    sched_freq_monthly_day: `A specific date each month`,
    sched_day_of_week: `Day of week`,
    sched_mon: `Monday`, sched_tue: `Tuesday`, sched_wed: `Wednesday`, sched_thu: `Thursday`,
    sched_fri: `Friday`, sched_sat: `Saturday`, sched_sun: `Sunday`,
    sched_day_of_month: `Day of month`,
    sched_run_time: `Time (WIB)`,
    sched_endless: `No end date (send indefinitely)`,
    sched_end_date: `End date (last send)`,
    sched_detail_title: `Schedule detail`,
    sched_subject_ph: `Subject`,
    sched_body_ph: `Body`,
    sched_confirm_email_label: `Your email (a confirmation code will be sent here)`,
    sched_send_code: `Send confirmation code`,
    sched_code_label: `Confirmation code`,
    sched_confirm: `Confirm & create schedule`,
    fpe_recipients_title: `Recipients`,
    fpe_recipients_hint: `Search registered users to add their email, or paste a list below — either or both.`,
    fpe_picked_label: `Picked recipients`,
    fpe_paste_label: `Or paste emails (one per line, or comma-separated)`,
    fpe_report_title: `Fund performance report`,
    fpe_hint: `Attaches the Reksa Dana Update PDF (all fund types, NAV as of the date above) to one email per recipient.`,
    fpe_log_title: `Send history`,
    fpe_log_hint: `Every fund performance email sent from this tab — who sent it, and to whom. Most recent first.`,
    fpe_modal_title: `Email fund performance`,
    ...
    ov_aum_as_of: `Platform AUM as of`,
    ov_all_funds: `All funds`,
    ov_funds_selected: `{n} fund{s} selected`,
    ov_fund_filter_hint: `Platform AUM uses its own as-of date above and only counts active funds, independent of the transaction date range at the top of the page. The fund filter scopes the KPIs and charts above the Largest funds table to the selected fund(s); total, verified, and new user counts stay platform-wide, and the Largest funds table has its own fund picker.`,
    ...
    refprog_period: `Referral program period`,
    refprog_hint: `Only counts an invitee's very first-ever transaction: it must be a Sucor Asset Management fund purchase of at least Rp1,000,000, made using another user's referral code, within the period above. Both the inviter and invitee must be KYC-verified, and the invitee's units in that fund must not decrease for 30 days after the purchase (checked daily against the custodian snapshot) for the bonus to become Eligible.`,
    refprog_summary: `Summary`,
    refprog_leaderboard_title: `Inviter leaderboard`,
    refprog_leaderboard_hint: `One row per inviter. "Invited" and "Transacted" count everyone they referred, any fund or amount — broader than "≥ Rp1jt", "Pending" and "Eligible", which only cover invitees whose first-ever transaction met the campaign's own product/amount rule.`,
    refprog_alt_leaderboard_hint: `One row per inviter. Unlike the main Referral program tab, "Invited" here is gated by the invitee's KYC verification date instead of their registration date (same 1-day grace window) — the referral itself can be arbitrarily old, but an invitee who's never been verified never counts as invited.`,
    refprog_invited_title: `Invited users`,
    refprog_invited_hint: `One row per invitee behind the leaderboard's "Invited" count — who invited them (SID and name), their own account details (including registration and KYC verification dates), referral code, the referrer code they signed up with, KYC status, and first-transaction status.`,
    refprog_detail_title: `Referral detail`,
    refprog_detail_hint: `One row per successful invitee. "Baseline units" is the unit balance right after the qualifying purchase; "Min units seen" dropping below that during the 30-day window is what flags a redemption.`,
    refprog_kpi_inviters: `Inviters`,
    refprog_kpi_invitees: `Successful invitees`,
    refprog_kpi_eligible: `Eligible`,
    refprog_kpi_pending: `Pending`,
    refprog_kpi_not_eligible: `Not eligible`,
    refprog_kpi_bonus_total: `Est. bonus payable`,
    refprog_kpi_bonus_total_sub: `Rp25,000 x 2 per eligible invitee`,
    ...
    evc_hint: `No dedicated event/code table exists yet, so this matches whichever of users.sales_code or users.referrer_code the event ends up using. From/To filters by registration date (when a user got tagged with the code) — it drives the funnel, cohort, and user list below. Transaction detail further down filters by transaction date instead.`,
    evc_funnel_title: `Funnel`,
    evc_funnel_hint: `Generic 4-step funnel: tagged with the code → KYC verified → made a transaction → made a repeat transaction. Swap in the event's real steps once it has its own rules.`,
    evc_kpi_tagged: `Tagged`,
    evc_kpi_verified: `KYC verified`,
    evc_kpi_transacted: `Transacted`,
    evc_kpi_repeat: `Repeat transacted`,
    evc_users_title: `Users tagged with this code`,
    evc_users_hint: `Paginated — a code can be a platform-wide default shared by hundreds of thousands of users, not necessarily a small personal cohort.`,
    evc_tx_hint: `Uses the same field/code(s) above, but its own date range — on transaction date, not registration date.`,
    evc_cohort_reg_title: `Cohort retention — by registration date`,
    evc_cohort_reg_hint: `Cohort = the period a tagged user registered in (Day/Week/Month, pick above — also applies to the first-transaction cohort below). Includes every tagged user, even ones who never transacted. % retained = users from that cohort who made a transaction in each later period, up to 12 periods out.`,
    evc_cohort_tx_title: `Cohort retention — by first transaction date`,
    evc_cohort_tx_hint: `Cohort = the period a tagged user's first transaction landed in — same basis as the Predict tab's retention cohorts. Only includes tagged users who transacted at least once. % retained = users from that cohort who made another transaction in each later period, up to 12 periods out.`,
    ...
    kpi_total_investors: `Total investors`,
  ...
  id: {
    ...
    reset_password_ph: `Kata sandi baru`,
    ...
    docs_presentation_desc: `Materi presentasi rapat internal, bisa ditampilkan layar penuh satu halaman per satu, terbagi jadi dua tab sidebar yang aksesnya bisa diberikan terpisah: AI Taskforce (Juli, September) dan Monthly Review (Agustus, September). Tidak ditampilkan untuk semua orang — hanya akun yang oleh admin sudah diberi akses ke tab tersebut (mis. login khusus untuk dibagikan saat rapat) yang bisa melihatnya, sama seperti bagian terbatas lainnya.`,
    ...
    ss_send_portfolio: `Portofolio per tanggal`,
    ss_send_statement: `E-statement transaksi untuk bulan`,
    ...
    ss_log_title: `Riwayat pengiriman`,
    ss_log_hint: `Setiap e-statement yang dikirim dari tab ini — siapa yang mengirim, dan ke investor mana. Terbaru di atas.`,
    ss_batch_title: `Kirim batch`,
    ss_batch_hint: `Kirim dokumen portofolio/e-statement yang sama ke banyak investor sekaligus — masukkan satu email atau SID per baris, atau dipisah koma.`,
    ss_batch_label: `Email atau SID`,
    ss_batch_modal_title: `Email statement batch`,
    sched_title: `Pengiriman otomatis`,
    sched_new: `+ Jadwal baru`,
    sched_hint: `Pengiriman berkala — jadwal baru hanya dibuat setelah orang yang mengaturnya mengonfirmasi kode sekali pakai yang dikirim ke emailnya.`,
    sched_modal_title: `Jadwal otomatis baru`,
    sched_recipient_type: `Penerima`,
    sched_recipient_single: `Satu email`,
    sched_recipient_csv: `Unggah CSV / tempel email`,
    sched_recipient_all_aum: `Semua investor dengan AUM`,
    sched_recipient_all_registered: `Semua pengguna terdaftar`,
    sched_single_email_label: `Alamat email`,
    sched_csv_label: `Unggah CSV, atau tempel email (satu per baris, atau dipisah koma)`,
    sched_send_portfolio: `Portofolio (live, per tanggal kirim)`,
    sched_send_statement: `E-statement transaksi (bulan sebelumnya)`,
    sched_frequency: `Frekuensi`,
    sched_freq_daily: `Harian`,
    sched_freq_weekly: `Mingguan`,
    sched_freq_monthly_last: `Akhir bulan`,
    sched_freq_monthly_day: `Tanggal tertentu setiap bulan`,
    sched_day_of_week: `Hari`,
    sched_mon: `Senin`, sched_tue: `Selasa`, sched_wed: `Rabu`, sched_thu: `Kamis`,
    sched_fri: `Jumat`, sched_sat: `Sabtu`, sched_sun: `Minggu`,
    sched_day_of_month: `Tanggal`,
    sched_run_time: `Waktu (WIB)`,
    sched_endless: `Tanpa tanggal berakhir (kirim terus-menerus)`,
    sched_end_date: `Tanggal berakhir (kirim terakhir)`,
    sched_detail_title: `Detail jadwal`,
    sched_subject_ph: `Subjek`,
    sched_body_ph: `Isi`,
    sched_confirm_email_label: `Email Anda (kode konfirmasi akan dikirim ke sini)`,
    sched_send_code: `Kirim kode konfirmasi`,
    sched_code_label: `Kode konfirmasi`,
    sched_confirm: `Konfirmasi & buat jadwal`,
    fpe_recipients_title: `Penerima`,
    fpe_recipients_hint: `Cari pengguna terdaftar untuk menambahkan emailnya, atau tempel daftar di bawah — bisa salah satu atau keduanya.`,
    fpe_picked_label: `Penerima terpilih`,
    fpe_paste_label: `Atau tempel email (satu per baris, atau dipisah koma)`,
    fpe_report_title: `Laporan performa produk`,
    fpe_hint: `Melampirkan PDF Reksa Dana Update (semua jenis produk, NAB per tanggal di atas) ke satu email per penerima.`,
    fpe_log_title: `Riwayat pengiriman`,
    fpe_log_hint: `Setiap email performa produk yang dikirim dari tab ini — siapa yang mengirim, dan ke siapa. Terbaru di atas.`,
    fpe_modal_title: `Email performa produk`,
    ...
    ov_aum_as_of: `AUM platform per tanggal`,
    ov_all_funds: `Semua produk`,
    ov_funds_selected: `{n} produk dipilih`,
    ov_fund_filter_hint: `AUM platform memakai tanggal acuannya sendiri di atas dan hanya menghitung produk yang aktif, terpisah dari rentang tanggal transaksi di bagian atas halaman. Filter produk membatasi KPI dan grafik di atas tabel Produk terbesar ke produk yang dipilih; total, terverifikasi, dan pengguna baru tetap mencakup seluruh platform, dan tabel Produk terbesar punya pemilih produknya sendiri.`,
    ...
    refprog_period: `Periode program referral`,
    refprog_hint: `Hanya menghitung transaksi pertama investor tersebut sepanjang masa: harus berupa pembelian produk Sucor Asset Management minimal Rp1.000.000, menggunakan kode referral orang lain, dalam periode di atas. Pengundang maupun teman yang diundang harus sudah KYC terverifikasi, dan unit teman yang diundang di produk tersebut tidak boleh berkurang selama 30 hari sejak pembelian (dicek harian dari snapshot kustodian) agar bonus menjadi Eligible.`,
    refprog_summary: `Ringkasan`,
    refprog_leaderboard_title: `Leaderboard pengundang`,
    refprog_leaderboard_hint: `Satu baris per pengundang. "Invited" dan "Transacted" menghitung semua teman yang diundang, produk/nominal apa pun — lebih luas dari "≥ Rp1jt", "Pending", dan "Eligible" yang hanya mencakup teman yang transaksi pertamanya memenuhi aturan produk/nominal kampanye.`,
    refprog_alt_leaderboard_hint: `Satu baris per pengundang. Berbeda dari tab Referral program utama, "Invited" di sini ditentukan oleh tanggal verifikasi KYC teman yang diundang, bukan tanggal pendaftarannya (toleransi tetap 1 hari) — referralnya sendiri boleh sudah sangat lama, tapi teman yang belum pernah terverifikasi tidak akan pernah dihitung invited.`,
    refprog_invited_title: `Pengguna yang diundang`,
    refprog_invited_hint: `Satu baris per teman yang diundang di balik angka "Invited" pada leaderboard — siapa yang mengundang (SID dan nama), detail akunnya sendiri (termasuk tanggal daftar dan tanggal verifikasi KYC), kode referral miliknya, kode referrer yang dipakai saat mendaftar, status KYC, dan status transaksi pertamanya.`,
    refprog_detail_title: `Detail referral`,
    refprog_detail_hint: `Satu baris per teman yang berhasil diundang. "Baseline units" adalah saldo unit tepat setelah pembelian yang memenuhi syarat; "Min units seen" yang turun di bawah itu selama jendela 30 hari menandai adanya penarikan dana.`,
    refprog_kpi_inviters: `Pengundang`,
    refprog_kpi_invitees: `Teman berhasil diundang`,
    refprog_kpi_eligible: `Eligible`,
    refprog_kpi_pending: `Pending`,
    refprog_kpi_not_eligible: `Tidak eligible`,
    refprog_kpi_bonus_total: `Estimasi bonus dibayarkan`,
    refprog_kpi_bonus_total_sub: `Rp25.000 x 2 per teman yang eligible`,
    ...
    evc_hint: `Belum ada tabel event/kode khusus, jadi ini mencocokkan users.sales_code atau users.referrer_code, mana pun yang nanti dipakai event. Dari/Sampai memfilter berdasarkan tanggal registrasi (saat pengguna ditandai dengan kode ini) — dipakai untuk funnel, kohort, dan daftar pengguna di bawah. Detail transaksi di bagian bawah memfilter berdasarkan tanggal transaksi.`,
    evc_funnel_title: `Funnel`,
    evc_funnel_hint: `Funnel generik 4 tahap: ditandai dengan kode → terverifikasi KYC → bertransaksi → bertransaksi ulang. Ganti dengan tahapan asli event begitu aturannya sudah ada.`,
    evc_kpi_tagged: `Ditandai`,
    evc_kpi_verified: `Terverifikasi KYC`,
    evc_kpi_transacted: `Bertransaksi`,
    evc_kpi_repeat: `Transaksi ulang`,
    evc_users_title: `Pengguna yang ditandai dengan kode ini`,
    evc_users_hint: `Dipaginasi — sebuah kode bisa jadi nilai default platform yang dipakai ratusan ribu pengguna, tidak selalu kelompok kecil milik satu orang.`,
    evc_tx_hint: `Memakai field/kode yang sama di atas, tapi rentang tanggalnya sendiri — berdasarkan tanggal transaksi, bukan tanggal registrasi.`,
    evc_cohort_reg_title: `Retensi kohort — berdasarkan tanggal registrasi`,
    evc_cohort_reg_hint: `Kohort = periode saat pengguna yang ditandai mendaftar (pilih Hari/Minggu/Bulan di atas — berlaku juga untuk kohort transaksi pertama di bawah). Mencakup semua pengguna yang ditandai, termasuk yang belum pernah bertransaksi. % bertahan = pengguna dari kohort itu yang bertransaksi di tiap periode berikutnya, sampai 12 periode ke depan.`,
    evc_cohort_tx_title: `Retensi kohort — berdasarkan tanggal transaksi pertama`,
    evc_cohort_tx_hint: `Kohort = periode saat transaksi pertama pengguna yang ditandai terjadi — basis yang sama dengan kohort retensi di tab Predict. Hanya mencakup pengguna yang ditandai yang sudah pernah bertransaksi minimal sekali. % bertahan = pengguna dari kohort itu yang bertransaksi lagi di tiap periode berikutnya, sampai 12 periode ke depan.`,
    ...
    kpi_total_investors: `Total investor`,
